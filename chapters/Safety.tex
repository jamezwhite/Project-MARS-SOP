...
Crew members must quickly recognize such abnormal behavior, check if the autopilot still responds to override commands, and be prepared to switch to manual control or trigger flight termination if the aircraft cannot be contained within the safe area.
        ...
        \subsection{Ground Risk Incursion}\label{ssec:ep-groundrisk}
            ...
            \begin{itemize}
                \item Visual observation by the \ac{rpic}, \ac{vo}, or other crewmembers of a person, vehicle, or animal entering the restricted operating area.
            ...
            \end{itemize}
            Once a ground incursion is noticed, the \ac{rpic} (or any crewmember) should immediately assess the position and movement of the intruder relative to the \ac{uas}. Key considerations include whether the \ac{uas} is on approach to that area, how much time is available to react, and what evasive action will best protect both the person and the aircraft.
            ...
            \subsubsection{Immediate Actions (All Flights)}
                ...
                \paragraph{} If crewmembers are available, have one attempt to warn or guide the intruder away from the danger (shout, wave, or approach if it can be done safely) while the \ac{rpic} keeps the \ac{uas} clear.
